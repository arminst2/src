...
@Component({
  selector: 'app-edit-medjuskladiste',
  templateUrl: './edit-medjuskladiste.component.html',
  styleUrls: ['./edit-medjuskladiste.component.css']
})
export class EditMedjuskladisteComponent implements OnInit {
  closeResult:string='';
  id:number = 0;
  racun!: IRacun;
  ...
  canDeactivate(): boolean {
    if(JSON.stringify(this.racun) !== JSON.stringify(this.racunZaPoredit)){
      if (confirm("Imate nesačuvane promjene! Ako napustite stranicu, promjene će biti izgubljene.")) {
        return false;
      } else {
        return true;
      }
 ...
 //search
  ...
  async addStavka(id: any){
    ...
    this.stavka.vpc = this.stavkaVPCRabatArtikal
    this.stavka.vpv = this.stavkaVPCRabatIznos
    this.stavka.mpv = this.stavkaMPCRabat
    this.stavka.dokumentId = this.racun.dokumentId;
    this.stavka.ulaznaCijena = this.currentArticleVPC;
  ...
  async updateRacun(){
    ...
    await this._racunService.updateRacunPromise(this.racun.racunId,this.racun);
    this.toastService.showSuccess('Račun uspješno ažuriran!', 'Uspješno ste ažurirali račun!');
  ...
  rabatCalc(){
    if (this.stavka.kolicina != 0 && this.currentArticleVPC != 0) {
      ...
      // console.log(this.stavkaIznos)
      // console.log((this.stavka.kolicina * this.currentArticleVPC))
  ...
  getArtiklById(id: any){
    this.artikl = this.artikli.find(x => x.artiklId == id);
    this.currentArticleVPC=this.artikl.vpc;
    this.stavkaVPCRabatArtikal = this.artikl.vpc
    this.stavkaVPCRabatArtikal = Number.parseFloat(this.stavkaVPCRabatArtikal.toFixed(2));
    var group = this.grupe.find(x => x.grupaId == this.artikl.grupaId);
    var porez = this.porezi.find(x => x.porezId == group!.porezId);
      ...
      content: [
        ...
        },
        {
          columns: [
            ...
            // }, 
            {
              text: `Međuskladišnica: ${this.racun.brojRacuna}`,
              alignment: 'center',
              margin: [0, 10, 0, 10],
              style: 'fontSizeL'
            }],
            ...
            },
            {
              columns: [
                ...
                },
                {
                  stack: [
                    {
                      text: this.skladista.find(f => f.skladisteId == this.racun.skladisteIzlazId)!.naziv,
                      alignment: 'right',
                      style: 'fontSize'
                    },
                    {
                      text: this.skladista.find(f => f.skladisteId == this.racun.skladisteUlazId)!.naziv,
                      alignment: 'right',
                      style: 'fontSize'
                    },
                    {
